...
class GestionPersonne {
    ...
    public GestionPersonne(Personne personne, Film film, RoleFilm roleFilm, Serie serie, RoleEpisode roleEpisode) throws Tp4Exception {
    ...
    public void ajoutPersonne(String nom, Date dateNaissance, String lieuNaissance, int sexe) throws Exception {
        try {
            ...
            if (personne.existe(nom)) {
                throw new Tp4Exception("Impossible d'ajouter, la personne " + nom + " existe deja.");
            }
    ...
    public void supprimerPersonne(String nom) throws Exception {
        try {
            ...
            if (!personne.existe(nom)) {
                throw new Tp4Exception("Impossible de supprimer, la personne " + nom + " n'existe pas.");
            }
            // S'il est le realisateur d'au moins un film
            if(!film.filmDeRealisateur(nom).isEmpty()) {
                throw new Tp4Exception("Impossible de supprimer, la personne " + nom + " a realise au moins un film.");
            }
            // S'il est le realisateur d'au moins une serie
            if(!serie.serieDeRealisateur(nom).isEmpty()) {
                throw new Tp4Exception("Impossible de supprimer, la personne " + nom + " a realise au moins une serie.");
            }
            // S'il a un role dans au moins un film
            if (!roleFilm.rolesDeActeur(nom).isEmpty()) {
                throw new Tp4Exception("Impossible de supprimer, la personne " + nom + " a un role dans au moins un film.");
            }
            // S'il a au moins un role dans au moins un episode d'au moins une serie
            if (!roleEpisode.rolesDeActeur(nom).isEmpty()) {
                throw new Tp4Exception("Impossible de supprimer, la personne " + nom + " a un role dans au moins une series.");
            }
    ...
    public void afficherFilmDeActeur(String nom) throws Tp4Exception, SQLException{
        if(!personne.existe(nom)){
            throw new Tp4Exception("Impossible d'afficher, l'acteur " + nom + " n'existe pas.");
        }
